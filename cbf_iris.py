...
# This is an exploratory Cultural-Bacterial Foraging prototype.
# It extends the simplified BF implementation with situational knowledge.
# It is not yet the complete Cultural Algorithm or thesis CBF method.
...
# Situational knowledge stores the best bacterium found so far.
belief_best_index = np.argmin(fitness_values)
belief_best_bacterium = bacteria[belief_best_index].copy()
belief_best_fitness = fitness_values[belief_best_index]
belief_updates = 0
...
for iteration in range(1, num_iterations + 1):
    ...
    for i in range(population_size):
        ...
        cultural_direction = belief_best_bacterium - bacteria[i]
        cultural_direction_norm = np.linalg.norm(cultural_direction)
        if cultural_direction_norm > 0:
            cultural_direction = cultural_direction / cultural_direction_norm

        combined_direction = (
            direction
            + cultural_influence * cultural_direction
        )
        combined_direction_norm = np.linalg.norm(combined_direction)
        if combined_direction_norm == 0:
            continue

        combined_direction = combined_direction / combined_direction_norm
    ...
    # Update situational belief knowledge before health accumulation.
